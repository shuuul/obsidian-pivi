...
- `build-css.mjs` — Prepends the Obsidian host theme-token mapping, concatenates the ordered `packages/pivi-react/styles/manifest.mjs` modules into root `styles.css`, and validates missing/unlisted CSS modules. Production mode minifies while preserving the Style Settings metadata block.
