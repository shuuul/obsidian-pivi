...
- `check-pi-pins.mjs` — Fails when `@earendil-works/pi-*` versions are ranged or desynchronized across root/`pivi-agent-core` manifests, the lockfile, and `piCodingAgentConfig.VERSION`. Wired into `npm run check:boundaries` through `npm run check:pi-pins`.
- `smoke-obsidian.mjs` — Deterministic real Obsidian/Electron lifecycle smoke (`npm run smoke:obsidian`): reload, open view, disposable note/session, fake stdio leak check, `window.fetch` identity, `dev:errors`. Requires Obsidian CLI and `OBSIDIAN_VAULT`.
