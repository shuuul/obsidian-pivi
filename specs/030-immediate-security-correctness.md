...
- [ ] OAuth callback responses never interpret authorization-server input as HTML, accept only `GET`, send explicit UTF-8 content types, and apply `Content-Security-Policy`, `X-Content-Type-Options`, `Cache-Control`, and `Referrer-Policy`.
- [ ] A stdio MCP child inherits only a documented cross-platform minimum environment plus explicitly configured server values; sensitive unrelated parent variables are absent.
- [ ] MCP HTTP/SSE URLs accept only `http:` and `https:`; non-loopback plaintext HTTP policy is explicit and test-covered.
- [ ] MCP server names reject `__proto__`, `prototype`, and `constructor`; serialized maps cannot mutate an object prototype.
- [ ] Editing a stdio MCP server preserves the exact executable and argument array, including whitespace, quotes, backslashes, and empty arguments.
- [ ] Process exits caused by a signal cannot be reported as exit code `0`; callers receive an explicit signal-aware failure result.
- [ ] Until spec `032` removes the global fetch patch, its user agent uses the current manifest/package version from one build-owned source.
- [ ] Opening settings, loading inventory, and automatic prefetch do not spawn stdio MCP processes.
- [ ] User-visible validation and storage wording is localized in every locale catalog.
- [ ] Focused tests, typecheck, lint, boundaries, build, plugin reload, and runtime error inspection pass.
...
|---|---|---|---|---|---|
| WS-01 | Harden OAuth callback method handling, response encoding, escaping, and browser security headers | Unassigned | Pending | None | Callback tests with script/img/entity payloads and non-GET requests |
| WS-02 | Replace ambient MCP stdio environment inheritance with a minimal allowlist and preserve lazy startup | Unassigned | Pending | None | MCP env and fake-process settings/prefetch tests on POSIX/Windows fixtures |
| WS-03 | Add strict MCP URL/name/header validation and exact command/args editing | Unassigned | Pending | None | Parser, storage, and React editor round-trip tests |
| WS-04 | Make process signal results truthful, bound output for current callers, and derive the temporary fetch user agent from the build version | Unassigned | Pending | None | Process runner signal/output tests and node-fetch header tests |
| WS-05 | Synchronize i18n, docs, guidance, and patch-release verification | Unassigned | Pending | WS-01–WS-04 | Full gates, build, reload, `obsidian dev:errors` |
...
npm run test -- --runInBand tests/pivi-react/McpToolsSection.test.tsx
npm run typecheck
npm run lint
