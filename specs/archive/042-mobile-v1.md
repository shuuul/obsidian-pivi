...
- [ ] Mobile supports provider setup, authenticated streaming chat, stop/cancel, retry, model selection, main-Agent tool calls, subagent execution that uses only Mobile-safe capabilities, and durable session create/open/append/fork/archive/delete flows.
...
- [ ] `manifest.json` changes to `isDesktopOnly: false` only after all Mobile startup, secret, transport, capability, storage, UI, and real-device release gates pass.
...
- Mobile-safe main-Agent/subagent registries; Vault tools; remote web search, image generation, and remote HTTP/SSE MCP only when their transport and UI pass the same Mobile gates.
...
| Remote web search and image generation | Target | Included only if transport, secret, budget, and result UI gates pass |
| Subagents | Target | Mobile-safe tool provider only; no inherited desktop authority |
| Remote HTTP/SSE MCP | Target | No stdio; transport/auth/OAuth paths must independently pass Mobile gates |
| Skills and Pivi Commands already stored in the Vault | Target | Read/use through Vault APIs; package CLI install/update is unavailable |
...
| 2026-08-09 | Keep `isDesktopOnly: true` until the complete Mobile release gate passes. | Removing the manifest guard before startup and artifact safety are proven would expose users to a plugin that crashes during load. | WS-01, WS-08 |
...
│ stdio + remote MCP      │     │ remote MCP only         │
...
3. Mobile bundle analysis rejects Node built-ins, Electron, process runners, OS path/session adapters, stdio MCP, CLI, and desktop-only dynamic import literals.
...
| Session journal/index/cache | Obsidian local storage or another Mobile-safe device-local adapter | No | Rebuildable/device-timing state must not conflict through sync |
...
|---|---|---|---|---|---|
| WS-01 | Define platform capabilities, split desktop/Mobile composition, add a Mobile build/artifact boundary, and retain the manifest guard | Unassigned | Pending | None | Architecture fixtures, Mobile production build, forbidden-import/metafile scan, desktop build |
| WS-02 | Move session create/open/list/append/fork/delete and related path ownership onto Vault-relative `FileStore`; provide Mobile-safe device-local journal/index storage | Unassigned | Pending | WS-01 contracts | Existing Pi fixtures plus shared in-memory DataAdapter and sync-replacement tests |
| WS-03 | Execute the real iOS/Android provider transport spike, choose the Mobile HTTP/stream implementation, and document security deltas | Unassigned | Pending | WS-01 | Device matrix with timestamps/provider/version, transport tests, cancellation/deadline/error evidence |
| WS-04 | Wire Mobile provider/settings flows to device-local state and SecretStorage; add exhaustive no-secret persistence/log/session tests | Unassigned | Pending | WS-01, WS-03 | Two-device isolation and sentinel-secret absence tests |
| WS-05 | Compose exact Mobile main/subagent tool, MCP, Skills, Commands, prompt, and Settings capability projections | Unassigned | Pending | WS-01–WS-04 | Exact inventory tests prove required/target presence and excluded capability absence |
| WS-06 | Adapt chat, settings, approval, session navigation, overlays, keyboard/IME, safe areas, and lifecycle UI for phone/tablet | Unassigned | Pending | WS-01, WS-02, WS-04, WS-05 | Component/DOM tests plus required human visual sign-off |
| WS-07 | Add Mobile lifecycle, storage, transport, and regression integration coverage while preserving desktop platform/security gates | Unassigned | Pending | WS-02–WS-06 | Focused suites, full quality gates, desktop Obsidian smoke |
| WS-08 | Run iOS/Android acceptance, finalize included target capabilities, update docs/guidance, flip `isDesktopOnly`, and prepare rollback/release evidence | Unassigned | Pending | WS-01–WS-07 | Real-device checklist, clean install/upgrade, release artifact inspection, `check:specs` |
...
Before release, inspect the final Mobile artifact and manifest, then repeat the desktop real-host smoke. `isDesktopOnly: false` is the last product-enabling change, not the first test mechanism. Rollback is restoring the manifest guard and publishing a corrected release; no secret or session migration may make rollback destructive.

## Documentation sync

- Numbered developer docs: update `docs/02-architecture-and-technology.md`, `docs/04-settings-storage-and-migration.md`, `docs/05-tabs-sessions-and-history.md`, `docs/07-tools-and-safety.md`, `docs/08-presentation-and-settings.md`, and `docs/10-roadmap-release-and-maintenance.md` with delivered Mobile behavior and boundaries.
- Nearest local guidance: update every changed area's nearest `AGENTS.md`, especially `src/`, `src/app/`, `packages/obsidian-host/`, `packages/pivi-agent-core/src/engine/pi/`, `packages/pivi-react/`, `build/`, `scripts/`, and `tests/` where applicable.
- Parent/package guidance: update affected package `AGENTS.md` and README public API/dependency boundaries when composition or exports change.
- Root guidance and roadmap: update root `AGENTS.md` platform overview and `docs/10-roadmap-release-and-maintenance.md` release/rollback state.
