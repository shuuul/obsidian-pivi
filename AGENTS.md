...
- **Settings search compatibility**: `PiviSettingTabHost.getSettingDefinitions()` maps `SETTINGS_ROOT_LAYOUT` to Obsidian 1.13 native pages and groups. Each page is a declarative `SettingDefinitionPage` whose single `items` entry is a `render` item with localized `name`, `desc`, and per-page aliases from `getSettingsPageSearchAliases`; that item mounts `mountSettingsPage` into `setting.settingEl`. General content on the root uses the same render item. There is no `SettingPage` subclass and no `display()` fallback. Locale changes call `update()` so the index is rebuilt. Plugin unload disposes any surface still mounted.
