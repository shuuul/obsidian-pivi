...
- Both release publishing paths must generate GitHub artifact attestations for `main.js`, `manifest.json`, and `styles.css` from `refs/tags/<version>` before uploading/re-uploading assets. Keep `id-token: write` and `attestations: write` permissions on the tag-bound artifact-upload job so users can verify both release asset integrity and tag provenance.
