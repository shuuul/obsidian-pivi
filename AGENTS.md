...
- **Optional Bash tool**: `obsidian_bash` is disabled by default and controlled by the Bash tool toggle (`allowBash`) plus `bashAllowlist`. It runs one allowlisted executable plus argument vector without a login shell, matches canonical executable paths and argument schemas (not command-string prefixes), rejects shell control syntax, and constrains cwd to the vault before invoking `@pivi/obsidian-host/systemProcessRunner`.
