...
- **High-risk operations**: Delete, overwrite, bulk mutation above the documented threshold, Bash/eval, first stdio MCP launch, and MCP artifact writes require turn-scoped confirmation bound to session/turn/kind/normalized resources. Fail-closed on denial/timeout/cancel/session switch/unload; subagents inherit-only and cannot open hidden approval UI. Metadata-only bounded audit lives under `.pivi/diagnostics/`.
