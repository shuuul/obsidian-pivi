...
- `packages/pivi-agent-core/src/engine/pi/piChatRuntime.ts` is the concrete `PiChatService`.
  ...
  - Manual `/compact` and threshold-based compaction summarize older JSONL entries through an auxiliary Pi agent and append a Pi compaction entry. Valid deterministic sections also produce `details.piviCheckpoint`; chained writes merge durable decisions/artifacts and render the merged ledger back into the unchanged plain-summary compatibility field. Invalid/unsafe structured output stays summary-only. Live chunks and restored-session mapping normalize valid checkpoints into the foundation-owned `CheckpointPresentation`; React never reads engine/session schema types. Per-session token estimates are cached with prefix sums; provider usage remains authoritative when explicitly marked, while fallback estimates distinguish ASCII, CJK/non-ASCII, code/JSON, tool structure, and images. Automatic and preflight decisions use the shared context envelope, including output, compaction, and safety reserves; pressure is the larger of provider-reported and current local estimates.
...
- `packages/pivi-agent-core/src/engine/pi/piAiModels.ts` owns the shared mutable pi-ai model registry. It installs explicitly supported built-in providers (including xAI), wires credentials/auth context, and configured custom providers. Interactive OAuth providers delegate login/logout to pi-ai through `piAuthInteraction.ts`. Chat context-window presentation resolves user overrides first, then the selected cached/registry model metadata, and returns an explicit unknown result when the model cannot be resolved; it must not invent a fallback limit.
...
- `packages/pivi-agent-core/src/engine/pi/piProviderOAuthService.ts` owns interactive provider OAuth for OpenAI Codex, xAI, and Anthropic: pi-ai `login`/`logout`, refresh-aware token lookup, and legacy Codex auth migration through injected OAuth and storage ports. `piAuthInteraction.ts` maps pi-ai `AuthInteraction` onto Pivi's OAuth host (browser URLs, device-code pages, and manual-code prompts that wait for localhost callbacks).
...
- `packages/pivi-agent-core/src/engine/pi/session/` implements those contracts using Pi JSONL and `SessionManager`:
  ...
  - `piContextCompaction.ts` owns token estimation, compaction thresholds, cut-point selection, checkpoint section parsing/merge/rendering, and summary prompts.
