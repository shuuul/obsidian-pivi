...
- `packages/pivi-agent-core/src/engine/pi/piChatRuntime.ts` is the concrete `PiChatService`.
  ...
  - Transient assistant failures use the Pivi-owned retry state machine rather than pi-coding-agent `AgentSession`. Context overflow remains owned by the existing compaction preflight; other upstream-classified network, rate-limit, timeout, and 5xx failures retain each failed assistant message in JSONL, remove only that failure from active model context, and retry through `Agent.continue()` after abortable 2/4/8-second delays. Three retries are allowed. `retry_start` / `retry_end` are transcript-neutral orchestration chunks that may drive transient progress chrome, and the error becomes visible as a message only when the policy settles on a terminal failure.
  ...
  - Automatic compaction uses a fixed bounded 85% trigger and starts an invisible Pass 1 ten context-window percentage points earlier. Pi's `buildContextEntries`, `findCutPoint`, and message conversion select the active context and preserve a safe weighted 95/5 turn/tool split as structured model input. Pass 1 summarizes the prefix into an in-memory `NOTE₁`; Pass 2 combines it with the raw tail and produces the validated full-replacement `NOTE₂`. `/compact [instructions]` keeps the same flow and applies instructions only to Pass 2. Both passes use the current chat model, no tools, low thinking, cancellation, and a 120-second timeout; failed sampling uses the bounded full-context fallback and never writes invalid output.
...
- `packages/pivi-agent-core/src/engine/pi/session/` implements those contracts using Pi JSONL and `SessionManager`:
  ...
  - `messageMapper.ts` reconstructs visible Pivi messages, tool calls, images, and UI overlays from Pi entries. `subagentMessageRecovery.ts` repairs a missing or incomplete `message_ui` subagent projection from Pi-native `spawn_agent` input/result fields, while a complete persisted `SubagentInfo` remains authoritative.
