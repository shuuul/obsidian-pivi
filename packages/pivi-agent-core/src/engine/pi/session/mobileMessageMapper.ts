...
import { extractDiffData } from '../../../tools/diff';
import {
  extractResolvedAnswers,
  extractResolvedAnswersFromResultText,
} from '../../../tools/toolInput';
import {
  isWriteEditTool,
...
export function entriesToChatMessages(
  ...
  compactionPresentation?: (branch: SessionEntry[], entryIndex: number) => Pick<
    Extract<ContentBlock, { type: 'context_compacted' }>, 'checkpoint' | 'tokensAfter'
  >,
): ChatMessage[] {
    ...
    if (entry.type === 'compaction') {
      ...
      const message: ChatMessage = {
        ...
        contentBlocks: [{
          ...
          ...(details && compactionPresentation ? compactionPresentation(branch, entryIndex) : {}),
