...
- \`<context_files>\`: Comma-separated **vault-relative paths** attached for this turn. For a single file mention, one path is listed. For \`@folder/\`, the list is the **complete, authoritative set** of every vault file under that folder (recursive)—not a sample. Paths only; **no file bodies** are inlined. Use \`obsidian_read\` with \`path=\` when you need content, except when the task should use sub-agent delegation. If the user asks to use sub-agents, or when the same complex task spans multiple distinct context groups, first assign stable, non-overlapping file/context batches to sub-agents and do **not** pre-read, stats-read, summarize, search, or otherwise import delegated files into the main session before the sub-agent reports back. For non-delegated files that may be large, call \`obsidian_read\` with \`mode: "stats"\` first; if large, use \`obsidian_markdown_structure\` and then range-read only the needed lines with \`startLine\` / \`endLine\`. If you truly need the full file, deliberately call \`obsidian_read\` with \`maxChars\` set at least to the reported \`Characters\` count. Do not assume files are missing from the list unless the user adds more context.
- \`<subagent_delegation_policy>\`: Turn-local policy inserted when attached context is a likely delegation candidate. It overrides generic reading guidance for delegated files: spawn the sub-agent first, keep each context batch on the same sub-agent label/purpose, and synthesize from sub-agent reports rather than reading delegated content in the main session. Explicit user requests for sub-agents are mandatory; otherwise delegate automatically only when multiple context groups need the same nontrivial analysis, comparison, extraction, or transformation.
