...
import { TOOL_SUBAGENT } from '@pivi/tools';
import { textResult } from '@pivi/tools/toolResult';

import type { PiRuntimeHost } from '../host/runtimeHost';
import { PiAuxQueryRunner } from '../PiAuxQueryRunner';

export function createSubagentTool(host: PiRuntimeHost): AgentTool {
  const runner = new PiAuxQueryRunner(host);
