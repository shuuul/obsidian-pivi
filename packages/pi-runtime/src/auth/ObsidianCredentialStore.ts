...
import { parseEnvironmentVariables } from '../settings/env';
...
function readLegacyPiAiCredential(secretStorage: SecretStorage, providerId: string): Credential | undefined {
  return parseStoredCredential(secretStorage.getSecret(getLegacyPiAiCredentialSecretId(providerId)) as string | null);
}
...
export class ObsidianAuthContext implements AuthContext {
  constructor(private readonly plugin: PiRuntimeHost) {}
