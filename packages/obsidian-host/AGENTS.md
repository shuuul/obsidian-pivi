...
- `src/obsidianVaultApi.ts` wraps Obsidian `App` vault operations: note reads/writes/edits, file resolution, tree/list, move/trash/folder creation, open-in-leaf, scan-based search that requires a Markdown note or non-root folder path, note info, links, backlinks, tag/graph analysis, property/alias indexes, base-file/view inspection, recent files, attachment metadata, and binary attachment creation with Obsidian markdown links. `writeNote` prepend inserts after YAML frontmatter; `inline` concatenates without a newline. `getTags`/`getProperties`/`getAliases` accept an optional note scope (`file`/`path`/`active`). Base lookup uses direct path/metadata resolution, and unresolved-only graph analysis reads metadata without enumerating files. Mutating paths use `requireAgentVaultMutationPath` (containment + managed-namespace policy); delete/move-source/mkdir use recursive mode so ancestors of managed roots are rejected. Before editing, overwriting, appending, prepending, changing properties, moving, deleting, or restoring an existing `.md` / `.canvas`, the API requires `fileRecoverySnapshot.ts` to complete. `runCliMutation` serializes exact-path out-of-process mutations, rejects unsaved target-editor content, captures the same required snapshot, and skips snapshots for paths that do not exist yet. Folder move/delete recursively snapshots all supported descendants before host mutation; any failure aborts the whole operation. New paths and unsupported attachments need no File Recovery snapshot; attachment deletion still uses Obsidian trash.
...
- `src/fileRecoverySnapshot.ts` resolves the enabled `file-recovery` internal plugin and calls private `forceAdd(path, content)` for `.md` / `.canvas` files before Pivi vault mutations. It accepts optional editor content to capture unsaved changes instead of the cached file. Disabled/unavailable File Recovery, a missing private API, and capture failure are explicit blocking errors.
- `ObsidianVaultApi.prepareActiveNoteInsertion` binds the active Markdown editor, file, text, and selections before asynchronous template resolution. Insertion revalidates those identities before and after the snapshot and synchronously replaces the bound selection; changes abort rather than redirecting the write. Prepend preserves YAML delimiter line boundaries even in inline mode, including empty frontmatter.
