...
- `src/obsidian/readExternal.ts` defines `obsidian_read_external`; it reads files by absolute path, or by a vault-relative path resolved against the current vault, through the injected `ExternalFileApiLike`, with stats, automatically paged complete-line ranges, and large-file handling. Vault-contained paths are always allowed. Outside the vault the tool is gated by `allowExternalRead`; paths outside allowed roots prompt through `CapabilityApprovalPort`.
- `src/obsidian/listExternal.ts` defines `obsidian_list_external`; it lists direct children of a folder by absolute path, or by a vault-relative path resolved against the current vault. Vault-contained paths are always allowed. Outside the vault the tool is gated by `allowExternalRead`; paths outside allowed roots prompt through `CapabilityApprovalPort`.
