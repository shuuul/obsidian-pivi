...
- `src/obsidian/bash.ts` defines `obsidian_bash`; it is registered only when `allowBash` is enabled, tokenizes one single-line command into an executable plus argv, matches the allowlist by canonical executable path and argument schema, rejects shell control syntax, forbids login-shell startup (`-lc`), constrains cwd to the vault, and invokes the injected process runner with shell forbidden. Runtime high-risk wrapping requires turn-scoped confirmation before execution. Its schema describes it as a lowest-priority host diagnostic, never a vault file tool.
...
- `src/obsidian/readShared.ts` and `src/obsidian/readTypes.ts` own shared line-span, stats, and complete-line range pagination used by `readNote.ts` and `readExternal.ts`. Every content read clamps `maxChars` to a 1,000-character minimum so exhausted turn allowances and smaller explicit values can still advance to compaction. Explicit ranges that exceed `maxChars` return a bounded page plus `nextStartLine`; only an individually oversized first line fails.
