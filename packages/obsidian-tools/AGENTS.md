...
- `src/obsidian/tasks.ts` defines `obsidian_tasks` (CLI-backed; no public task index API). List accepts `status=`, `verbose`, `total`, and `active`; mutation includes `action: status`. `src/obsidian/templates.ts` and `src/obsidian/bookmarks.ts` define CLI-only `obsidian_templates` / `obsidian_bookmarks`. `src/obsidian/command.ts` defines `obsidian_command`: it is registered whenever the official CLI is active and discovers commands and hotkeys with read-only `list` / `hotkey` / `hotkeys` actions. `execute` additionally requires `allowCommand`; an exact command-ID grant executes immediately and a missing grant uses the shared approval flow.
