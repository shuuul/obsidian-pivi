...
- Every CLI path that mutates an existing note (`history:restore`, task status changes, daily append/prepend, and template-based create with overwrite) calls `ObsidianVaultApi.captureSnapshotBeforeCliMutation` first. Snapshot failure blocks the CLI call. New note creation needs no snapshot, and delete remains trash-only with no permanent-delete parameter.
...
- `src/obsidian/base.ts` defines `obsidian_base`; list/views actions use the vault API and `.base` YAML parsing, while query and create remain explicitly CLI-backed.
