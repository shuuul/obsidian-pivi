...
- `src/createObsidianTools.ts` constructs the full `ToolSpec[]` from an Obsidian `App`, settings, and optional image generator. When the vault path is known, it registers `obsidian_read_external` / `obsidian_list_external` and injects the vault root as an implicit allowed directory so vault files (including `.pivi/skills`) can be read and listed without a Settings grant; that root is not listed in Persistent permissions. `allowExternalRead` only gates paths outside the vault.
