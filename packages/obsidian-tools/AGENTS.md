...
- `src/obsidian/tasks.ts` defines `obsidian_tasks` (CLI-backed; no public task index API). List accepts `status=`, `verbose`, `total`, and `active`; mutation includes `action: status`. `src/obsidian/templates.ts` and `src/obsidian/bookmarks.ts` define CLI-only `obsidian_templates` / `obsidian_bookmarks`. `src/obsidian/command.ts` defines `obsidian_command`: it is registered whenever the official CLI is active and discovers commands and hotkeys with read-only `list` / `hotkey` / `hotkeys` actions. `execute` additionally requires `allowCommand` and explicit command-ID membership in `commandAllowlist`; an empty list denies all execution.
...
- Every CLI path that mutates an existing note (`history:restore`, task status changes, daily append/prepend, template insertion, and template-based create with overwrite) calls `ObsidianVaultApi.captureSnapshotBeforeCliMutation` first. Snapshot failure blocks the CLI call. New note creation needs no snapshot, and delete remains trash-only with no permanent-delete parameter.
...
- Mutating vault operations execute directly; optional capabilities are setting-gated: command execution uses `allowCommand` plus an explicit command-ID allowlist, Bash uses `allowBash` plus structured persistent permissions, and filesystem tools outside the vault use `allowExternalRead` plus allowed external directory roots. Vault-contained absolute paths skip that grant.
