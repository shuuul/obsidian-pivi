...
- `src/obsidian/readNote.ts`, `writeNote.ts`, `editNote.ts`, `search.ts`, `listPath.ts`, `mkdir.ts`, `movePath.ts`, `deletePath.ts`, `openPath.ts`, `noteInfo.ts`, `links.ts`, `properties.ts`, and `attachment.ts` define the core vault tools (`read`, `write`, `edit`, `search`, `ls`, `mkdir`, `move`, `delete`, `obsidian_open`, `obsidian_note_info`, `obsidian_links`, `obsidian_properties`, `obsidian_attachment`) over the injected vault API; mutating paths go through host `requireAgentVaultMutationPath` (containment + Pivi-managed MCP/Skills/Commands namespace rejection with `pivi_*` tool guidance). `read` and `ls` route internally: vault-indexed paths use the Vault API, and an index miss that still exists on disk (or an absolute path) uses ExternalFileApi (`unmanagedVaultPath.ts`). `edit` owns literal exact replacement, including local newline insertion, delimiter removal/repositioning, and explicit `replaceAll: true`. Multiple `edits[]` items match the original file, not incrementally; overlapping spans fail. Its prompt usage must teach shortest-unique-substring newline edits and explain that surrounding text remains adjacent and Markdown block markers require physical-line boundary handling. `search` `promptUsage` owns search-not-read: locate notes and match positions; `context: true` dumps are not a substitute for reading note bodies. `path` is required and must be one Markdown note or a non-root folder; omitted/empty/`/`/vault-root paths are rejected and do not call `searchNotes`. Listing queries (`*`, `**`, empty, `path:`-only) error toward `ls`. Serialized search results share the `ls` 50,000-character ceiling, a hard `limit` of 200, truncated match-context lines, and a continuation marker. CLI fallback is folder-only and offset-zero because the official command cannot preserve a single-note scope or pagination. `ls` filters direct-child names with an optional case-insensitive substring `query`, then pages the filtered rows with a default 50-entry / maximum 200-entry 0-based `offset`/`limit`, returns `nextOffset`, and enforces a 50,000-character serialized-result ceiling so one large folder cannot overflow the next model request. `write` `content` and each `edit` `oldText`/`newText` reject values over 50,000 characters. `write` `prepend` inserts after YAML frontmatter; `inline` concatenates without a newline; `template=` creates through official CLI `create` and always passes the validated exact `path`. `obsidian_open` accepts `file=` or `path=` and maps `newtab` to `target=tab`. CLI fallbacks (`search`, `links`, `noteInfo`, `history`, `tasks`, `base` query, `daily`, `templates`, `bookmarks`, `command`) share `cliOutput.ts` so stdout is capped at 50,000 model-visible characters. `obsidian_tags` list/info JSON is capped the same way.
- `src/obsidian/tasks.ts` defines `obsidian_tasks` (CLI-backed; no public task index API). List accepts `status=`, `verbose`, `total`, and `active`; mutation includes `action: status`. `src/obsidian/templates.ts` and `src/obsidian/bookmarks.ts` define CLI-dependent `obsidian_templates` / `obsidian_bookmarks`. `src/obsidian/command.ts` defines `obsidian_command`: it is registered whenever the official CLI is active and discovers commands and hotkeys with read-only `list` / `hotkey` / `hotkeys` actions. `execute` additionally requires `allowCommand`; an exact command-ID grant executes immediately and a missing grant uses the shared approval flow.
- `src/obsidian/history.ts` defines `obsidian_history`; it uses the Obsidian CLI history commands to list, read, restore, and `diff` stored file versions, including deleted files when history exists. Restore asks the host vault API to snapshot an existing destination first and blocks CLI mutation if that required snapshot fails; a deleted destination has no current state to capture. Because that mandatory snapshot is prepended as the new version 1 and the CLI resolves `version=<n>` against the shifted list at execution time, restore lists the versions before the mutation, re-anchors the requested number inside the mutation callback by counting prepended entries through full-list alignment, verifies the restore write landed (version list grew and the new entry carries the selected version's size), and fails loudly with re-list guidance when the lists cannot be aligned or the restore did not apply — never restoring a stale number and never reporting an unverified success. `file=` or `path=` is required except for `files`.
- Every CLI path that mutates an existing note (`history:restore`, task status changes, and template-based create with overwrite) runs through `ObsidianVaultApi.runCliMutation`. The host serializes each exact path, blocks unsaved active-editor content, and captures a File Recovery snapshot immediately before the CLI call. Daily append/prepend resolves the exact path through read-only CLI and writes through the Vault API. New note creation needs no snapshot, and delete remains trash-only with no permanent-delete parameter.
- Templates require CLI for listing/reading/resolution, but insertion is not CLI-only: bind the host Markdown editor before read-only `template:read`, then snapshot its unsaved content and replace its selection in-process. Editor, file, content, or selection changes abort insertion; never call mutating `template:insert`.
- `obsidian_properties` list/read/aliases JSON shares the 50,000-character result ceiling. `write` prepend preserves the YAML closing-delimiter line boundary even with `inline: true`; inline concatenation only applies to the body.
- `src/obsidian/daily.ts` defines `obsidian_daily`; it uses the official Obsidian CLI to resolve/read the daily note, then uses the Vault API for append/prepend without depending on daily-notes internals.
...
- `src/obsidian/tags.ts` defines `obsidian_tags`; it lists tags and tag details through the injected vault API / MetadataCache, without shelling out. List can scope to `file`/`path`/`active`.
- `src/obsidian/base.ts` defines `obsidian_base`; list/views actions use the vault API and `.base` YAML parsing, while query remains explicitly CLI-backed. Item creation is unavailable because the CLI does not expose the generated note path before mutation.
