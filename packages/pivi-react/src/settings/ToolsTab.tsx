...
import { SettingHeading, SettingRow, SettingsPageDescription, Toggle } from './controls';
...
export function ToolsTab({ ports }: { readonly ports: SettingsPorts }) {
  ...
  return <><SettingsPageDescription><p className="pivi-setting-description">{t('settings.tools.intro', { hostName })}</p></SettingsPageDescription>{error ? <div className="pivi-setting-description" role="alert">{error}</div> : null}<SettingHeading>{t('settings.externalRead.heading')}</SettingHeading><SettingRow name={t('settings.externalRead.allow.name')} description={t('settings.externalRead.allow.desc')}><Toggle checked={settings.allowExternalRead} disabled={pending} label={t('settings.externalRead.allow.name')} onChange={(allowExternalRead) => { void save({ allowExternalRead }); }} /></SettingRow><div className="pivi-external-directories-setting pivi-setting-stack"><SettingRow name={t('settings.externalRead.directories.name')} description={t('settings.externalRead.directories.desc')}><textarea className="pivi-settings-external-dirs-textarea" rows={4} cols={40} value={directories} placeholder={t('settings.externalRead.directories.placeholder')} disabled={pending} onChange={(event) => setDirectories(event.target.value)} onBlur={() => { void commitDirectories(); }} /><button type="button" title={t('settings.externalRead.directories.browseTooltip')} disabled={pending} onClick={() => { void chooseDirectory(); }}>{t('settings.externalRead.directories.browse')}</button></SettingRow></div><SettingHeading>{t('settings.bash.heading')}</SettingHeading><SettingRow name={t('settings.bash.allowlist.name')} description={t('settings.bash.allowlist.desc')}><textarea rows={4} cols={40} value={bashAllowlist} disabled={pending} onChange={(event) => setBashAllowlist(event.target.value)} onBlur={commitBashAllowlist} /></SettingRow><SettingHeading>{t('settings.tools.heading')}</SettingHeading>{ports.complex.tools.listToolRows().map((row) => {
