...
- `scripts/build-css.mjs` prepends `packages/obsidian-host/styles/pivi-theme.css`, then resolves the 41 manifest paths within this directory, concatenates modules in declaration order, and writes root `styles.css`.
...
- `settings/`: shared settings navigation/layout plus provider registry, command editor, slash, MCP, prompt, and agent-specific rules. `settings/base.css` owns only shared settings infrastructure; feature-specific rules live in their named modules.
...
- Compact control feedback uses `.pivi-settings-action-feedback` beside its owning action: host green for success and host error red for failure. `.pivi-settings-action-group` and `.pivi-settings-control-feedback` provide shared wrapping/layout; feature styles must not recreate page-level result banners. Installed-skill row status (`.pivi-sp-item > .pivi-settings-action-feedback`) is a full-width wrap below the toggle/update/remove cluster so pending and success copy stay on that skill instead of trailing the delete button or jumping to the list heading.
- Settings page descriptions, row content, section labels, and list-header titles align to `--pivi-settings-gutter`. `SettingsSection` owns asymmetric section spacing (`--pivi-settings-section-gap` before, `--pivi-settings-section-title-gap` after the label). The page uses the host primary background rather than the paler settings/secondary surface, while rounded primary section bodies consume the host settings-item surface backed in Obsidian by `--setting-items-background` with a public primary-alt fallback; primary section and collection headings use a subtle bottom divider. Nested subsections inside a section body use `--pivi-settings-subsection-gap` without another enclosing surface. Section headings reset host `h2`/`h3` margin/padding under `.pivi-settings`. Lists and cards stay full width with internal padding; feature CSS must not introduce one-off row insets. A list header owns the header-to-list gap, so the following list must not add another top margin.
