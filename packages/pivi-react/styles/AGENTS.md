...
- `settings/features/`: page-owned structure only (no spacing, radius, or border declarations; `color` / `background*` must be a single `--pivi-settings-*` or `--pivi-host-*` token). Populated by later settings page workstreams.
...
- Settings page descriptions, row content, and section labels align to `--pivi-settings-gutter`. `SettingsSection` owns asymmetric section spacing (`--pivi-settings-section-gap` before, `--pivi-settings-section-title-gap` after the label) with a quiet muted heading and hairline divider; there is no surfaced section body. Nested subsections use `--pivi-settings-subsection-gap`. Section headings reset host `h2`/`h3` margin/padding. Rows are flat and separated by hairline dividers. `DisclosureCard` is the only surfaced settings element. Feature CSS must not introduce one-off row insets.
