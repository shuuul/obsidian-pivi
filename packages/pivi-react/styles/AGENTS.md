...
- `scripts/build-css.mjs` prepends `packages/obsidian-host/styles/pivi-theme.css`, then resolves the 48 manifest paths within this directory, concatenates modules in declaration order, and writes root `styles.css`.
...
- `settings/system/`: settings UI contract — `tokens.css` (the only file that may declare `--pivi-settings-*`), `host.css` (definition-row reset and page surface), `layout.css` (page/section rhythm), `row.css`, `controls.css`, `card.css`, `feedback.css`. These seven files load immediately after `modals/mcp-modal.css` and before the legacy `settings/*.css` modules.
- `settings/features/`: page-owned structure only (no spacing, radius, or border declarations; `color` / `background*` must be a single `--pivi-settings-*` or `--pivi-host-*` token). Models checklist/logo slot lives in `features/models.css`.
- `settings/`: legacy settings modules still consumed by unmigrated cards and the tab shell (`base.css`, provider/command/slash/MCP/prompt/agent). Deleted in WS-06 once pages are on the primitives.
...
- Settings form controls share `.pivi-settings-control` for sizing, borders, focus, placeholder, and disabled states. Use `--fill` only for intentional full-width editors; checkbox, toggle, and range controls are excluded. Badge-list fields own wrapping, long-token overflow, and compact per-badge remove buttons in `settings/system/controls.css`.
- Compact control feedback uses `.pivi-settings-feedback` beside its owning action: host green for success and host error red for failure. `.pivi-settings-action-group` and `.pivi-settings-control-feedback` provide shared wrapping/layout; feature styles must not recreate page-level result banners. Installed-skill row status (`.pivi-sp-item > .pivi-settings-feedback`) is a full-width wrap below the toggle/update/remove cluster so pending and success copy stay on that skill instead of trailing the delete button or jumping to the list heading.
- Settings page descriptions, row content, and section labels align to `--pivi-settings-gutter` (the same token as row name padding). `SettingsSection` uses the interface font and native heading metrics, a hairline under the heading, then one surfaced rounded body (`--pivi-settings-surface-background` / `--pivi-host-setting-items-background`) holding flat rows and disclosure rows separated by hairlines. A section nested inside another section body or a `DisclosureCard` body emits `pivi-settings-section--nested`: no surface, smaller heading, subsection gap. Nested subsections use `--pivi-settings-subsection-gap`. Section headings reset host `h2`/`h3` margin/padding and must not inherit the vault heading font. `DisclosureCard` is a disclosure row inside that surface (no own border, radius, or background); header order is icon · name · summary · badges · actions · chevron. `SettingsInlineActions` has no chrome of its own. Feature CSS must not introduce one-off row insets.
...
- Composer context meter styles live in `components/context-footer.css`; the usage ring shows total prompt context and the optional cache ring to its left uses `.pivi-meter-fill-cache` (`--pivi-host-text-success`). There is no output ring class or inspector styles. Obsidian owns each ring's `data-tooltip`; the stylesheet must not duplicate it with a pseudo-element or add a hover/active shadow.
...
- Prompt-tab startup-context usage (stacked bar, estimate rows, over-budget warning) and the modified badge live in `settings/prompt-settings.css`.
- Shared provider-card whole-header pointer sorting, keyboard drag handles, priorities, transform-only drag feedback, and reduced-motion fallback live in `settings/provider-settings.css`; Web and model provider lists use the same sorting chrome, while screen-reader-only reorder status stays in `base/presentation-primitives.css`. Open `.pivi-provider-card` / `.pivi-mcp-card` disclosures must keep `overflow: visible` (toolbar cards already do); `overflow: hidden` on an open `<details>` clips nested compatibility fields and turns the card into a hidden scrollport so the settings pane cannot reach the footer. `.pivi-settings-definition-host` in `settings/system/host.css` resets the Obsidian 1.13 definition row to auto height with visible overflow, leaving scrolling on the native settings page so its scrollbar stays at the window edge. Custom-provider Display name / Base URL rows (`.pivi-provider-endpoint-fields`) give the control the leftover row width instead of the 280px right-aligned cap; the 520px stacked-row breakpoint clears that info-column max-width. Open `.pivi-settings-card.is-open` must also keep `overflow: visible`.
