...
- Activity rows use static profile/tool icons plus one trailing status mark. Only the running status arc uses continuous motion; do not restore animated header gradients or per-profile looping motion. Reduced-motion mode stops the arc while preserving icon, text, and color semantics.
