...
- Commands settings show built-in commands such as `/compact` in a read-only Internal commands section, separate from editable/deletable workspace custom commands. Runtime SDK commands and skills do not belong in the internal section.
...
- MCP server cards list the enabled tools exposed to the slash selector as compact badges. The `/` marker denotes composer slash-badge support. MCP verification presents all discovered tools as a responsive checklist grid with explicit enable-all/disable-all actions; tool toggles are optimistic but must roll back visibly when persistence fails.
