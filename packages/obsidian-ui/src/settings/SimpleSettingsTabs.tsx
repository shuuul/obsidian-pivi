...
export function IntegrationsSettingsTab({ actions }: { readonly actions: SettingsActionsPort }) {
  const t = useT();
  const [pending, setPending] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const mounted = useMountedRef();
  const setup = async (itemStyle: 'label-and-icon' | 'icon-only') => {
    setPending(true);
    try {
      const result = await actions.setupNoteToolbarIntegration(itemStyle);
      if (mounted.current) {
        setMessage(t(
          `settings.noteToolbar.${result.status === 'installed' ? 'installed' : result.status === 'already-installed' ? 'alreadyInstalled' : 'failed'}`,
  ...
  const openStyleSettings = async () => {
    const opened = await actions.openStyleSettings();
    if (!opened && mounted.current) setMessage(t('settings.styleSettings.installationOpened'));
  };
  return (
    <>
      <SettingHeading>{t('settings.styleSettings.name')}</SettingHeading>
      <SettingRow name="" description={t('settings.styleSettings.desc')}>
        <button type="button" onClick={() => { void openStyleSettings(); }}>{t('settings.styleSettings.open')}</button>
      </SettingRow>
      <SettingHeading>{t('settings.noteToolbar.heading')}</SettingHeading>
      <SettingRow name="" description={t('settings.noteToolbar.desc')}>
        <button type="button" disabled={pending} onClick={() => { void setup('label-and-icon'); }}>
          {t('settings.noteToolbar.setupLabelAndIcon')}
        </button>
        <button type="button" disabled={pending} onClick={() => { void setup('icon-only'); }}>
          {t('settings.noteToolbar.setupIconOnly')}
        </button>
      </SettingRow>
      {message ? <div className="setting-item-description">{message}</div> : null}
    </>
