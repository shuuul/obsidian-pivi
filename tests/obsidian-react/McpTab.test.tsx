...
import { createI18n, I18nProvider, SettingsRoot } from '@pivi/obsidian-react';
import type { SettingsPorts } from '@pivi/obsidian-react/ports';
import type { SettingsUiSnapshotData } from '@pivi/obsidian-react/settings';
...
function makePorts(initial: ManagedMcpServer[] = []) {
  ...
  const ports: SettingsPorts = { snapshot: { getSnapshot: () => snapshot }, actions: { saveGeneral: async () => undefined, saveSubagents: async () => undefined, purgeDeletedSessionFiles: async () => 0, openStyleSettings: async () => true, setupNoteToolbarIntegration: async () => ({ status: 'installed' }) }, complex: { mcp } as unknown as SettingsPorts['complex'], persistence: { getSettingsSnapshot: () => ({} as never), commitSettingsSnapshot: async () => undefined }, environment: { getActiveEnvironmentVariables: () => '', getEnvironmentVariables: () => '', applyEnvironmentVariables: async () => undefined, applyEnvironmentVariablesBatch: async () => undefined, getReviewKeys: () => [] }, hotkeys: { listHotkeys: () => [], openHotkeySettings: () => undefined }, catalog: { listModelsForProvider: () => [], syncCustomProviders: () => undefined, fetchCustomProviderModels: async () => ({ count: 0 }) } };
  return { ports, mcp, getServers: () => servers };
}

async function openMcp(ports: SettingsPorts) { render(<I18nProvider i18n={createI18n()}><SettingsRoot ports={ports} /></I18nProvider>); fireEvent.click(screen.getByRole('tab', { name: 'MCPs' })); await act(async () => undefined); }
