...
describe('subagent shell styles', () => {
  ...
  it('keeps the header and icon geometry stable while toggling', () => {
    ...
    expect(styles).not.toMatch(/\.pivi-subagent-card:not\(\.expanded\) \.pivi-subagent-header/);
  ...
  it('keeps the subagent name stable and gives the brief description the remaining width', () => {
    ...
    expect(styles).not.toMatch(/\.pivi-subagent-card:not\(\.expanded\) \.pivi-subagent-label/);
  });

  it('uses the shared small shell radius while collapsed and expanded', () => {
    expect(styles).toMatch(/\.pivi-subagent-card\s*\{[^}]*border-radius:\s*var\(--pivi-radius-sm\);/s);
    expect(styles).not.toMatch(/\.pivi-subagent-card:not\(\.expanded\)\s*\{[^}]*border-radius:/s);
    expect(styles).not.toMatch(/\.pivi-subagent-card\.expanded\s*\{[^}]*border-radius:/s);
  });
