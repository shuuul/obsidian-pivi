...
import { HomeFileAdapter } from '../../../../src/core/storage/HomeFileAdapter';
...
describe('VaultFileAdapter', () => {
  function createAdapter(initialFiles: Record<string, string> = {}) {
    ...
    return { adapter: new VaultFileAdapter(app), files, writeMock };
  }
  ...
  it('allows later appends after a failed append', async () => {
    const { adapter, files, writeMock } = createAdapter({ 'log.jsonl': 'before\n' });
...
describe('HomeFileAdapter', () => {
  it('rethrows unexpected deleteFolder errors', async () => {
    ...
    jest.spyOn(adapter as unknown as { resolve(path: string): string }, 'resolve')
