...
function standardFixture(): Model<Api> {
  return { provider: 'openai', id: 'gpt-standard-fixture', reasoning: false } as Model<Api>;
}
...
describe('PiThinkingLevels (core)', () => {
  describe('getPiThinkingLevelOptionsForModel', () => {
    ...
    it('returns only off for non-reasoning models', () => {
      expect(getPiThinkingLevelOptionsForModel(standardFixture())).toEqual([expect.objectContaining({ value: 'off' })]);
    });

    it('includes descriptions for supported thinking levels', () => {
      const highOption = getPiThinkingLevelOptionsForModel(reasoningFixture()).find((option) => option.value === 'high');
    ...
    it('keeps upstream thinking-level order and labels model-specific maximum levels', () => {
      const options = getPiThinkingLevelOptionsForModel({ ...reasoningFixture(), thinkingLevelMap: { xhigh: 'xhigh', max: 'max' } });
      expect(options.map((option) => option.value)).toEqual(['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max']);
  ...
  describe('getPiDefaultThinkingLevelForModel', () => {
    ...
    it('uses a server-advertised default instead of medium', () => {
      const model = {
        ...reasoningFixture(),
        thinkingLevelMap: { off: 'none', minimal: null, high: null, max: null, low: 'low', medium: 'medium', xhigh: 'xhigh' },
        defaultThinkingLevel: 'xhigh',
      } as Model<Api>;
      expect(getPiThinkingLevelOptionsForModel(model).map((option) => option.value)).toEqual(['off', 'low', 'medium', 'xhigh']);
