...
import { VaultFileAdapter } from '../../../../src/core/storage/VaultFileAdapter';
import type { ManagedMcpServer } from '../../../../src/core/types';
import { McpOAuthService } from '../../../../src/pi/mcp/oauth/McpOAuthService';

function createMemoryVaultAdapter(): VaultFileAdapter {
  const app = new App();
  const files = new Map<string, string>();
  const folders = new Set<string>();

  app.vault.adapter.exists = jest.fn(async (path: string) => files.has(path) || folders.has(path));
  ...
  return new VaultFileAdapter(app);
}
...
describe('McpOAuthService', () => {
  ...
  it('scopes MCP OAuth tokens by server URL through created auth providers', async () => {
    const service = new McpOAuthService(createMemoryVaultAdapter());
    const originalProvider = service.createAuthProvider(oauthServer('github', 'https://mcp.example.com'));
    const movedProvider = service.createAuthProvider(oauthServer('github', 'https://other.example.com'));
