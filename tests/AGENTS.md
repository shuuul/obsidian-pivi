...
```mermaid
flowchart TD
  Runner["npm run test<br/>scripts/run-jest.js"] -- "loads config" --> Jest["jest.config.js<br/>projects: unit + jsdom"]
  Jest -- "Node unit project" --> Window["setupWindow.ts<br/>window + RAF shims"]
  Jest -- "jsdom project" --> React["jsdom + Testing Library<br/>setupObsidianUi.ts"]
...
- `unit/engine-pi/` — Pi engine auth, models, runtime, session, shims, and tool-adaptation tests.
- `unit/features/` — feature UI/service tests such as chat tab lifecycle and fork flows.
- `unit/agent/settings/` and `unit/agent/config/` — device-local provider/environment state and config-value source contract tests.
