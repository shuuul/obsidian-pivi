...
- Chat performance tests assert deterministic invariants rather than speculative speedups: one projection commit per fake animation frame, viewport-bounded mounted rows, stable entity identity, 100-message projection pages, safe Markdown sealing, and synchronous lifecycle flushes. Record timing/heap claims only from an explicit reproducible benchmark run.
