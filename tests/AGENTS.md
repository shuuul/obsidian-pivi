...
```mermaid
flowchart TD
  Runner["npm run test<br/>scripts/run-jest.js"] -- "loads config" --> Jest["jest.config.js<br/>projects: unit + integration + obsidian-react"]
  Jest -- "Node projects" --> Window["setupWindow.ts<br/>window + RAF shims"]
  Jest -- "obsidian-react project" --> React["jsdom + Testing Library<br/>setupObsidianUi.ts"]
...
- `obsidian-react/` — React/TSX behavior tests running in the dedicated jsdom project.
...
- Existing unit and integration projects run in Node. Only `tests/obsidian-react/**` runs in jsdom; keep DOM-heavy React tests there.
- Architecture fixtures lock the four ownership seams: core owns runtime/application ports, app owns concrete wiring, `@pivi/obsidian-react` owns React presentation, and `src/ui` owns remaining product orchestration and imperative adapters.
