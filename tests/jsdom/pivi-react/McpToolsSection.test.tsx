...
function makePorts(initial: ManagedMcpServer[] = []) {
  ...
  const ports: SettingsPorts = { snapshot: { getSnapshot: () => snapshot }, feedback: { notify: jest.fn() }, actions: { saveGeneral: async () => undefined, saveSubagents: async () => undefined, saveEditorSelectionToolbar: async () => undefined, loadSessionMaintenance: async () => ({ archivedCount: 0, deletedCount: 0 }), deleteAllArchivedChats: async () => ({ moved: 0, skippedActive: 0, failed: 0 }), purgeDeletedSessionFiles: async () => 0 }, complex: {
    tools: { getSettings: () => ({ allowBash: false, bashPermissions: [], allowExternalRead: false, externalDirectories: [] }), listToolRows: () => [], setToolEnabled: async () => undefined, chooseExternalDirectory: async () => null, validateExternalDirectory: async () => ({ valid: true }), saveSettings: async () => undefined },
