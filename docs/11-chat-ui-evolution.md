...
Pass 1 produces an in-memory vault-native `NOTE₁`. Pass 2 sends that draft together with the raw final 5% to the current chat model and produces a validated, self-contained `NOTE₂`. Manual `/compact [instructions]` runs both passes when no draft is available, and the instructions apply only to Pass 2. Sampling uses no tools, low thinking, a 120-second timeout, and the active chat model. Validation accepts the dedicated checkpoint fence plus whole-response JSON forms, requires the complete checkpoint schema, and rejects unsafe paths without imposing an arbitrary note-length floor. A failed pass enters a bounded full-context single-pass fallback; later retries explicitly restate the complete-object requirement, and no compaction entry is written when all attempts fail.
