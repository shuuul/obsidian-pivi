...
The composer meter starts after the first user→assistant exchange (provider usage if present, otherwise a full local estimate). After that, every tool result is added as trailing pressure **before** the next provider continuation — the continuation barrier — so the ring moves as soon as the tool returns. Overflow copy uses that projected pressure, not only the last successful assistant usage. If in-turn compaction cannot get under the 85% trigger, the continuation is aborted instead of sending an over-window request.
