...
Vault reads use the default configured directly below **Read (`obsidian_read`)** in Settings → Tools (100K characters initially) or explicit `maxChars`, capped at 500K characters. Other persistent tool-specific controls are colocated the same way: external-read access and roots below **Read external**, and the command allowlist below **Bash**. Invocation-only schema parameters remain Agent-controlled rather than becoming global settings. The allowance is fixed for a read rather than shrinking near the compaction trigger; each main or child Agent still owns an independent turn budget that prevents parallel sibling reads from each claiming the full ceiling. If a result pushes the session over the trigger, the existing compaction preflight handles it before the next provider request.
