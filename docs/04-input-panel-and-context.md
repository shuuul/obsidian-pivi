...
- Ask AI sends through `PiviChatViewHandle.commands.submitInlineEditTurn`, which creates an **archived** session tab without activating it. Each processed assistant text chunk is forwarded directly from the normal chat turn pipeline; inline edit does not poll tab state or wait for the turn promise to resolve before publishing output.
