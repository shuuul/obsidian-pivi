...
Each tab's message projection separately derives active top-level background Agent runs for the optional Active Work Shelf. The mounted view bridge combines those immutable rows across tabs; it never becomes an execution registry or persistence layer. Shelf navigation switches to the owning tab and asks the virtual transcript viewport to center the owning message. Existing inactive-tab attention state remains responsible for completed background work.

React owns message ordering and an empty content-adapter slot. The app bridge updates a single mounted imperative subagent renderer incrementally rather than remounting it on every chunk. Consecutive sibling runs instead render as one React-owned Agent Group summary backed by stable Agent-run projection IDs; expanding it reveals shared Activity rows while single runs retain the rich imperative adapter. Each grouped row can disclose an in-row linear inspector derived from the same durable nested tool sequence, including objective, prompt, child-run depth, and terminal text. Stored and live subagents converge on the same pure `ChatMessage` representation.
