...
Collapsed tool and subagent cards construct only their header chrome. First expansion parses and mounts the complete content already held by the snapshot; collapsed updates mark that body dirty, and reopening rebuilds from the latest snapshot. Presentation never replaces available content with `... N more`, while provider/tool truncation remains explicit and no expansion re-executes work or reads a new source. Each expanded top-level tool, steps group, or subagent wrapper is capped at one third of its actual messages viewport with `overflow: hidden` while its direct body child owns the sole internal scrollbar; the direct title is layout-fixed at the card top and never sticks to the transcript. Nested titles inside the body scrollport use sticky positioning with measured offsets. At the internal scroll end, default scroll chaining continues through the transcript without changing disclosure state, so the expanded card moves upward and is progressively clipped rather than disappearing in one collapse. Inside a subagent, the steps title sticks at `top: 0` in the content scrollport and tool titles stick below it at the measured steps height; top-level steps groups keep the steps title on the card while tools stick at `top: 0` inside the scrolling steps body.
