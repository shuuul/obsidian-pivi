...
Settings mount independently through `PiviSettingTabHost` and `SettingsRoot`. `getSettingDefinitions()` returns Obsidian 1.13 declarative pages and groups (`minAppVersion` 1.13.0) with one indexed `render` item per page that mounts `mountSettingsPage`. There is no `SettingPage` subclass and no `display()` fallback. Plugin unload disposes every live settings surface. The service graph does not contain a settings renderer.
