...
The Models page ends with a Command line section. Its "Default model for CLI runs" select persists the device-local `cliDefaultModel` preference through the shared settings persistence port; empty follows the current chat model, and provider removal or model-list refresh prune the value like the title model. The `pivi:run` CLI command consumes it when no `model=` parameter is passed.
