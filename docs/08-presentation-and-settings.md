...
MCP stdio editors keep servers inactive until the user confirms executable, arguments, working directory, and environment variable names (secret values are never shown). High-risk chat approvals use an app-owned localized modal outside React settings; approval copy lives under the shared `highRisk.*` i18n keys.
