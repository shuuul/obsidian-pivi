...
| Pivi `src/` (excl. i18n) | **~713 KB (~11%)** | similar |

**Deploy layout:** `main.js`, `manifest.json`, `styles.css` only (community install standard).

The plugin bundle is dominated by **Pi stack + provider SDKs**, not Pivi feature code. The largest win was **esbuild hoisting** bare imports from `pi-coding-agent/node_modules` to project root (see `esbuild.config.mjs` `dedupe-pi-coding-agent-nested` plugin).
...
| ~26 KB | `InputController.ts` | Largest Pivi TS module |
...
This is an **npm layout / bundler resolution** issue, not dead code in Pivi.
...
### 3. Do **not** expect big wins from splitting Pivi files
...
`pi-ai@0.80.x` supports explicit `Models` collections and per-provider factory imports. Pivi now uses `src/pi/piAiModels.ts` to select the supported provider factories instead of loading every pi-ai provider by default.
