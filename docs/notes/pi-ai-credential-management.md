...
Pivi has migrated the model catalog/auth plumbing so pi-ai provider APIs see Obsidian-backed credential services:

- `src/pi/auth/ObsidianCredentialStore.ts` implements pi-ai `CredentialStore` over Obsidian SecretStorage.
- Provider credentials are stored under one unversioned SecretStorage id per provider: `pivi-<provider>-credential`.
- Startup/settings rendering hard-migrates legacy `pivi-<provider>-credential-v2`, `pivi-<provider>-api-key`, `pivi-<provider>-oauth-token`, and plaintext provider env snippets into that canonical id, then clears the legacy slots/snippets.
...
Compatibility path still remains for runtime auth transport, not credential storage shape:

- `src/pi/runtime/piModelEnv.ts` resolves keys from Pivi settings/keychain/environment snippets.
...
This avoids forcing users to re-enter provider credentials while preventing new installs from accumulating multiple SecretStorage entries for the same provider.
...
3. Document which ambient filesystem credentials are supported by `AuthContext` on desktop and which remain opt-in/disabled in Obsidian.

## Caveats

- Pivi hard-migrates existing keychain IDs into the canonical provider credential id.
