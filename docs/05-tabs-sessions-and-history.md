...
Session identity, history summaries, usage, and UI context are also read through the JSONL index rather than a full Pi snapshot. Rebuildable indexes are device-local (outside synced `.pivi`) and recreate when missing or corrupt. The same self-healing applies on the write path: when a cloud sync replaces the session file between turns (new inode/mtime, identical bytes), the post-append index refresh rebuilds from the authoritative JSONL instead of aborting the turn, while the pre-write source-fingerprint guard still rejects any mutation during a live session's lifetime. Restored running asynchronous subagents are marked orphaned as each page appears. The Pi runtime still assembles complete model context from authoritative JSONL independently of this bounded UI projection.

### Cloud-file recovery

After each successful local JSONL append, Pivi seals the continuation into a vault-scoped device-local write-ahead journal (`pivi.session-journal.v1`) and marks it confirmed. The journal stores only continuation data already permitted in session persistence—no credentials, absolute external paths, or rebuildable UI caches. Startup reconciliation detects replacement, inode change, truncation, rollback, fingerprint divergence, interrupted append, and unacknowledged entries. Matching sources drop the journal row; append-compatible gaps reapply the sealed lines; non-append-compatible divergence creates an explicit recovered session/fork with visible provenance and leaves the externally changed JSONL untouched. Repeated recovery is idempotent via divergence identity. Recovery source rewrites are atomic (temp + rename with temp cleanup), the recovered-identity map is bounded, and a corrupt device-local journal blob resets to empty with a warning. The live stale-write guard remains in place for in-memory writers.
