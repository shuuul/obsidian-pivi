...
flowchart TD
  ...
  App -- "constructs" --> Engine["@pivi/engine-pi"]
  Chat -- "consumes" --> Runtime["@pivi/agent/runtime"]
  React -- "consumes presentation-safe models" --> Agent["@pivi/agent domain subpaths"]
  App -- "uses" --> Host["@pivi/obsidian-host"]
  App -- "uses" --> Tools["@pivi/obsidian-tools"]
  Tools -- "uses" --> Host
  Tools -- "implements" --> ToolSpec["@pivi/agent/tools ToolSpec"]
  App -- "adds agent session tool" --> ToolSpec
...
`PiChatRuntime` constructs the low-level Pi `Agent` directly rather than adopting pi-coding-agent `AgentSession`. `AgentSession` also owns persistence, compaction, tools, prompts, resources, models, and extensions, which overlaps Pivi's established boundaries. Its complete static dependency graph also expands the Obsidian artifact materially: with the then-installed `@earendil-works/pi-coding-agent@0.81.1` (the pin at measurement time; see the root and `@pivi/engine-pi` package manifests for the current exact pin), a production build experiment grew `main.js` from roughly 3.0 MiB to 7.9 MiB. Pivi therefore keeps provider recovery narrow: retryable network, rate-limit, timeout, and 5xx assistant failures (including Node TLS handshake disconnects and `ECONNRESET` that pi-ai's classifier currently misses) are discarded from durable/model history, their live partial projection is replaced, and the request is retried up to three times with abortable 2/4/8-second backoff. Legacy persisted error attempts remain inspectable history but are excluded from future LLM context. Context overflow remains under the existing compaction policy.

### Ports and dependency injection

Runtime, sessions, model catalogs, slash catalogs, and projected settings enter chat orchestration through `@pivi/agent`-owned `ChatPorts`. React settings use React-owned `SettingsPorts`. Host file, secret, HTTP, process, and storage capabilities are injected through `@pivi/agent/ports`. Explicit ports make ownership testable and prevent a wide plugin object from becoming a service locator.

Network egress is enforced at a shared transport boundary. Host-neutral policy lives in `@pivi/agent/network`; `@pivi/obsidian-host` implements purpose-scoped streaming HTTP clients with deadlines, byte limits, redirects, and DNS pinning. App composition installs those clients into Pi providers, MCP/OAuth, WebSearch/WebFetch, image generation, skills, and connectivity probes. Provider Total and body-idle deadlines use a live mutable policy object so settings changes affect subsequent requests without reconstructing the network clients; a zero value disables that timer while leaving all other egress limits intact. Pivi does not patch `window.fetch`; the production bundle injects a scoped `fetch` for upstream SDK call sites only. See [SECURITY.md](../SECURITY.md) for disclosure, trust boundaries, credential matrix, prompt-injection stance, and Skills/MCP responsibility.

Local process execution and vault mutation are similarly explicit host primitives. `ProcessRunRequest` requires byte limits, timeout, cwd policy, shell policy (forbidden by default), and optional abort; the host runner terminates owned process trees and reports termination kinds without double-resolve. Vault writes use `requireVaultRelativeMutationPath` separately from display/read normalization so absolute/UNC/traversal/symlink-parent escapes fail before Obsidian APIs run. Focused macOS/Windows CI covers path/process/MCP stdio/Skills suites; support claims do not exceed that tested behavior. See [SECURITY.md](../SECURITY.md).
...
| Concrete Pi construction | `@pivi/engine-pi`, `src/app/**` and `src/main.ts` only |
