...
`search` locates notes and match positions. `path` is required and must be one Markdown note or a non-root folder; omitted, empty, `/`, and vault-root paths are rejected. Serialized results share the `ls` 50,000-character ceiling, a hard `limit` of 200, truncated match-context lines, and a continuation marker. It is not a content-read channel: `context: true` dumps are not a substitute for reading note bodies.
