...
Mobile composition registers exactly 15 public-API Vault tools: read, Markdown structure, search, list, note info, links, properties, tags, graph, write, edit, move, delete, mkdir, and attachment. Mutations use the same approval boundary and managed `.pivi` namespace protections as Desktop; property reads/listing remain non-mutating, while set/remove requires approval. Abort, modal close, backdrop, Escape, and plugin cleanup all prevent a late approval from executing a mutation.

Mobile V1 does not register Bash/eval, Obsidian CLI, external paths, stdio or remote MCP, Skills installation/runtime, workspace Pivi Commands, subagents, web tools, or image generation. They are structurally absent from tools, prompts, catalogs, and Settings rather than visible-but-failing. The Mobile slash picker separately exposes only the Mobile-safe built-in chat commands `/new` and `/compact`; this does not load the workspace command or Skills subsystems. Future Mobile additions must enter through Mobile-safe leaf exports and pass their own transport, secret, lifecycle, and presentation gates.
...
Pivi's Desktop composition includes optional filesystem, process, and environment-backed integrations; Mobile composition excludes those authorities. Direct filesystem access is confined to explicit external roots, vault-local Pivi data, provider compatibility stores, and configured Skills/CLI paths. Vault mutations require canonical vault-relative paths via `requireVaultRelativeMutationPath` and reject absolute/UNC/traversal/symlink-parent escape before Obsidian APIs run. Process execution uses the bounded Desktop host `ProcessRunner` for CLI, allowlisted Bash, Skills distribution tooling, and related one-shot work; MCP stdio spawns a long-lived child with vault cwd and rejects shell-control characters in the executable field. Opening an external authentication URL remains a detached opener, not a shell. Environment values are read at those Desktop integration boundaries for provider credentials, MCP authentication/stdio variables, CLI discovery, and Skills tooling; Pivi does not collect or transmit machine identity to its author.
