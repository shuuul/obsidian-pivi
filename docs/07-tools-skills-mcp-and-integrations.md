...
Vault skills live under `.pivi/skills/`. The `skill` tool loads their instructions for an agent turn. Install/update/remove uses the exact pinned `skills` dependency (`node <cli.mjs>`, `shell: forbidden`), validates isolated operation trees (no symlinks/escapes; size/`SKILL.md` limits), then publishes atomically. Runtime and settings inventory reads tolerate a transiently missing or locked skill entry while an external copy is completing; the next refresh picks it up without blanking the surface. Open-view skill refresh is isolated per chat view so one disposed view cannot fail the publication commit. Agent-facing vault mutation policy reserves `.pivi/skills/**` plus every `skills-<operation>-*` staging, CLI-metadata, and transaction root, including remove/rollback roots. A first vault load may offer the `kepano/obsidian-skills` bundle, but installation and updates require explicit user confirmation. This repository does not track runtime vault skills.
