...
| Daily, graph, tags, Bases, templates, bookmarks | `obsidian_daily`, `obsidian_graph`, `obsidian_tags`, `obsidian_base`, `obsidian_templates`, `obsidian_bookmarks` | Depends on operation; daily path/read, templates, bookmarks, and Base query use the official CLI where required |
...
| Host execution | `bash`, `obsidian_command` | Command/hotkey discovery is read-only; command execution and Bash are potentially mutating and disabled by default |
...
`search` locates notes and match positions. `path` is required and must be one Markdown note or a non-root folder; omitted, empty, `/`, and vault-root paths are rejected. Matching is case-insensitive unless `caseSensitive` is true. Serialized results share the `ls` 50,000-character ceiling, a hard `limit` of 200, truncated match-context lines, and a continuation marker. It is not a content-read channel: `context: true` dumps are not a substitute for reading note bodies. CLI fallback is limited to folder searches with `offset: 0`, because the official command cannot preserve a single-note scope or the tool's pagination contract.
...
CLI mutations of existing note content run inside an exact-path host transaction. History restore, task status changes, and template-based overwrite are serialized per path, create a File Recovery snapshot immediately before the process call, and block when the active target editor has unsaved content; transaction or snapshot failure blocks the mutation. Daily append/prepend resolves the exact path through read-only CLI and mutates through the Vault API. Base item creation is unavailable because the CLI does not expose the generated note path before mutation. Template insertion instead binds the active Markdown editor, resolves the template through read-only CLI, snapshots the editor's current content, and replaces its selection in-process. A changed editor, file, content, or selection aborts insertion; no mutating `template:insert` CLI call is made. Delete remains an Obsidian trash operation and has no permanent-delete parameter.

`obsidian_properties` list/read/aliases output shares the 50,000-character text ceiling with tags and CLI results. Truncated results carry an explicit marker; narrow to `file`/`path` or omit `verbose` to reduce the result. `write` prepend preserves the YAML delimiter boundary even with `inline: true`, including empty frontmatter and a closing delimiter without a trailing newline.
...
- `obsidian pivi:run command=<name>` runs one Pivi workspace command (`.pivi/commands/`) headlessly and prints the model result. `file=<name>` resolves a note like a wikilink and `path=<path>` takes an exact vault path; the resolved content feeds the command's `{{current_note}}`, `{{current_note_name}}`, and `{{date}}` variables. `selection="<text>"` supplies `{{selected_text}}` for commands that require a selection. The `model=provider/model_id` parameter overrides the default model from Settings → Models; when both are unset the run follows the raw runtime default (empty). Results reuse the shared 50,000-character cap with an explicit truncation marker, and failures surface as CLI errors naming the fix (missing `command=`, unknown command plus available names, unresolvable note, missing selection, or an invalid `model=` shape).
- The default model is the shared default-model preference (the same active model new conversations start with), selected on the Models page under Default model using the composer-style model selector; its list shows exactly the checked models of enabled providers, like the composer.
