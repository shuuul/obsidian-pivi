...
CLI mutations of existing note content create a File Recovery snapshot before the process call. This covers history restore, task status changes, daily append/prepend, and template-based overwrite; snapshot failure blocks the mutation. Template insertion instead binds the active Markdown editor, resolves the template through read-only CLI, snapshots the editor's current content, and replaces its selection in-process. A changed editor, file, content, or selection aborts insertion; no mutating `template:insert` CLI call is made. Delete remains an Obsidian trash operation and has no permanent-delete parameter.
