...
- Make session persistence resilient to cloud-file replacement, starting with iCloud Drive: **implemented** via a vault-scoped device-local write-ahead journal (`pivi.session-journal.v1`), startup reconciliation that recovers rollback/divergence into an explicit recovered session instead of overwriting either source, and rebuildable JSONL indexes moved to `~/.pivi/session-indexes/<vault-key>/`. The stale-write guard remains. Recovery source rewrites are atomic, the recovered-identity map is bounded, and a corrupt device-local journal resets to empty with a warning. Compatibility means no silent overwrite and no loss of a locally completed turn; simultaneous multi-device writers may still produce an explicit fork rather than one fabricated linear history. See the [example-vault diagnostic](../specs/archive/019-live-session-source-mutation-diagnostic.md) and archived spec `035`.
