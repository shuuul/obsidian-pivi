...
- **Composable workflow prompts** (spec `046`): typed prompt-module registry (core locked, workflow composable including default-off long-line normalization, custom modules after workflow), Settings Prompt tab with compact estimated-token usage, synced `.pivi/settings.json` composition, and `estimateTextTokens` owned by `@pivi/agent/prompt` (engine compaction re-exports it).
...
- **Sidebar capability approvals** (spec `038`): unlisted Bash commands and external paths outside allowed roots prompt an inline Deny / once / session / always confirmation in the sidebar instead of a modal. Bash always-allow persists either an `exact:` complete command or, only for shell-safe single commands, a `prefix:` argv grant; external reads persist the directory root to device-local `externalReadDirectories`. Session grants clear on session switch.
- **Session cloud recovery** (spec `035`): vault-scoped device-local write-ahead journal (`pivi.session-journal.v1`), startup reconciliation that recovers rollback/divergence into an explicit recovered session instead of overwriting either source, and rebuildable JSONL indexes under `~/.pivi/session-indexes/<vault-key>/`. Recovery source rewrites are atomic, the recovered-identity map is bounded, and a corrupt device-local journal resets to empty with a warning. Compatibility means no silent overwrite and no loss of a locally completed turn; simultaneous multi-device writers may still produce an explicit fork rather than one fabricated linear history. See the [example-vault diagnostic](../specs/archive/019-live-session-source-mutation-diagnostic.md).
