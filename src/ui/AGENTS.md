...
`src/ui/` owns chat runtime orchestration, reusable imperative UI primitives, and inline-edit editor orchestration/selection bridging. The Obsidian-specific `@pivi/obsidian-react` package owns product chrome plus inline-edit CodeMirror state/widget types and mount/dispose; this layer keeps runtime coordination and product adapters that require Obsidian editor context.
...
flowchart TD
  ...
  AppUI -- "mount + React-owned SettingsPorts" --> ReactUI["@pivi/obsidian-react React roots"]
...
- Import `@pivi/obsidian-react` only through the exact `store`, `inline-edit`, or `context-badges` presentation subpath. Root-barrel, `mount`, `ports`, deep-internal, and relative-path package imports are forbidden; pure parsing, matching, markdown transforms, and usage projection come from core domain subpaths.
...
- Add keys to canonical `packages/obsidian-react/src/i18n/locales/en.json` and mirror the same key tree with translations in every other locale in the same change. Follow the package i18n guidance.
...
- Settings presentation is fully React-owned in `@pivi/obsidian-react`; do not reintroduce imperative settings managers under `src/ui/`.
...
- Avoid new `!important` styles and hard-coded English. CSS lives in `packages/obsidian-react/styles/`, not beside these TypeScript modules.
