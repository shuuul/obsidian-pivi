...
flowchart TD
  ...
  Tabs --> Input["input/<br/>RichChatInput + context-chip adapters"]
...
| `src/ui/chat/composer/` | Provider-neutral turn request construction, outgoing-turn setup/finalization, FIFO turn queueing/restoration, inline prompts, capability approval prompts and tab bridging, response duration, tool-paused generation clock for tokens/s, and pure ordered-Markdown-list continuation edits. | — |
...
| `src/ui/chat/input/` | Imperative adapters for uncontrolled rich input, file/image/inline context chips, and textarea sizing. `src/ui/chat/input/file-context/` has its own `AGENTS.md`. | `src/ui/chat/input/file-context/AGENTS.md` |
...
| `packages/pivi-react/src/mount/ChatShell.tsx` | React-owned header, logo, tabs, welcome/quote adapter slot, queue, composer toolbar (including usage and cache-hit meters), todo status, navigation, auto-scroll status, and owner-realm interactions. Consumes snapshots/actions, not application runtime ports. |
...
| `src/ui/chat/input/RichChatInput.ts` | Uncontrolled contenteditable adapter with textarea-compatible API, mention badges, canonical-token copy/cut, plain-text paste that rebuilds badges, ordered-list continuation, and IME-safe synchronization. React never owns its children. |

## Patterns and constraints

### Boundaries

- UI chat code depends on the `app`-only `PiviChatHost` contract from `src/app/hostContracts.ts`, not `PiviChatCompositionHost`, the concrete plugin class, `PiviViewHost`, or app runtime implementations. Do not import `@/app/ui/**` from this directory.
...
- Do not import `@pivi/engine-pi`, raw `@earendil-works/*` SDK modules, `src/app/runtime/**`, or `@pivi/obsidian-host/**` from this directory.
...
- External-context selections are session/turn capabilities. Reset session-only selections on new/load flows; synchronize pinned external roots across all views. Absolute roots and per-turn paths are overlaid from Obsidian's device-local cache and must never be written into synced settings or JSONL. MCP enable/disable lives in Settings; tab/view warmup and settings saves prefetch enabled HTTP/SSE servers. Stdio MCP is not supported.
...
- **Chunk ordering is semantic.** Text, thinking, tools, compact boundaries, and subagents must be finalized in arrival order or stored history will render differently from the live turn. The one deliberate exception: reasoning-leak thinking deltas that arrive after visible text merge back into the segment's open thinking run (`mergePostTextThinkingRuns` in the stream reducer, mirrored by `@pivi/engine-pi` messageMapper when reading stored overlays) so live and stored history converge on pi-native channel assembly.
...
- **Provider retries are engine orchestration with transient chrome projection.** `retry_start` / `retry_end` keep the foreground turn active and temporarily replace the bottom thinking indicator with the retry number plus owner-window countdown/active state. On `retry_start`, the provider-boundary handler replaces the failed in-progress assistant projection so partial failed text/tools cannot join the recovered answer. Cancellation/reset clears the indicator, and only an exhausted terminal `error` reaches `handleErrorChunk()`.
