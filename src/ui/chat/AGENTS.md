...
- **Provider retries are engine orchestration with transient chrome projection.** `retry_start` / `retry_end` keep the foreground turn active and temporarily replace the bottom thinking indicator with the retry number plus owner-window countdown/active state. They do not mutate the transcript projection; cancellation/reset clears the indicator, and only an exhausted terminal `error` reaches `handleErrorChunk()`.
