...
export class StatusPanel {
  ...
  private currentTodoModel: TodoVisualizationModel | null = null;
  ...
  remount(): void {
    ...
    if (this.currentTodos && this.currentTodos.length > 0) {
      this.updateTodoVisualization(this.currentTodoModel ?? deriveTodoVisualizationModel(this.currentTodos, 'manual'));
    }
    this.updatePanelVisibility();
  }
  ...
  updateTodos(todos: TodoItem[] | null): void {
    this.updateTodoVisualization(todos && todos.length > 0 ? deriveTodoVisualizationModel(todos, 'manual') : null);
  }

  updateTodoVisualization(model: TodoVisualizationModel | null): void {
    ...
    this.currentTodoModel = model;
    this.currentTodos = model?.items ?? null;
    ...
    const completedCount = model.progress.completed;
    const totalCount = model.progress.total;
    const currentTask = model.items.find(t => t.id === model.activeItemId);

    // Update header
    this.renderTodoHeader(completedCount, totalCount, currentTask);

    // Update content
    this.renderTodoContent(model);
   ...
   */
  private renderTodoContent(model: TodoVisualizationModel): void {
    if (!this.todoContentEl) return;
    renderTodoVisualizationPanel(this.todoContentEl, model);
  }
  ...
  private updateTodoDisplay(): void {
    ...
    if (this.currentTodos && this.currentTodos.length > 0) {
      const model = this.currentTodoModel ?? deriveTodoVisualizationModel(this.currentTodos, 'manual');
      const completedCount = model.progress.completed;
      const totalCount = model.progress.total;
      const currentTask = model.items.find(t => t.id === model.activeItemId);
      this.renderTodoHeader(completedCount, totalCount, currentTask);
