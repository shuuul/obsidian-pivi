...
- Composer serialization, copy/cut, and cursor mapping must agree on browser-created `div`/`p` line boundaries and empty-line `<br>` placeholders. Native selections can point to the editor or nested containers, not only text nodes. Preserve an already-placed composer selection when dropdowns return focus; otherwise adding another token can lose a badge or overwrite the wrong span.
...
- Inline badges are not the source of truth: `data-mention-token` is. Changing a visible label must not mutate submitted text. Copy and cut from `MentionInput` serialize that token (a selection that intersects a badge copies the whole token); paste inserts `text/plain` and rebuilds recognized badges. Inline-context badges are removable; ordinary inline mention badges generally are not.
- Inline badges clamp to `min(100%, 24ch)` with label ellipsis; the full path or token remains on `title`. Do not let a long file/folder/skill label stretch the composer line.
- Context badges render as `span.pivi-context-badge--inline` only in composer inline mode and as `button` otherwise. Keep the explicit inline class synchronized with the element mode so compact composer metrics apply. Navigation requires an explicit callback: shared `mentionBadgeNavigation` routes files through `openLinkTarget` (existing/deferred document tab or new tab), vault folders to Files, and slash commands through the app-registered settings navigator. External folder and tool badges remain inert. Private native-page and explorer access stays in `obsidianPrivateApi.ts`; unavailable targets report a Notice rather than replacing the current document.
