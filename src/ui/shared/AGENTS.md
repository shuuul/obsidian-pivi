...
- Floating overlays in `selectionToolbar/floatingOverlay.ts` pass Escape to the host only when invisible; visible overlays consume Escape. Register outside `pointerdown` in the capture phase and remove listeners on hide/dispose.
- Slash and mention dropdowns dismiss on outside `pointerdown`, cancel pending mention debounce timers on hide/destroy, and subscribe to owner scroll to reposition or dismiss anchored menus.
