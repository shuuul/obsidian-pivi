...
import type { App, Editor, MarkdownView, Plugin, PluginManifest } from "obsidian";
 ...
 */
export default class PiviDesktopRuntime implements PiviPluginHost {
  ...
  readonly platformCapabilities: PiviPlatformCapabilities;
  ...
  constructor(
    ...
    this.deviceLocalExternalContexts = new ObsidianDeviceLocalExternalContextStore(this.app);
    this.deviceLocalEnvironmentStore = new ObsidianDeviceLocalEnvironmentStore(this.app);
    this.sessionManager = new OpenSessionManager({
      getVaultPath: () => getVaultPath(this.app),
      getStore: () => this.requireSessionStore(),
    });
    this.workspaceCommandRegistry = new WorkspaceCommandRegistry(owner, this);
  ...
  async onload() {
    ...
    await initializePiviPlugin(this.owner, this);
    ...
    this.owner.registerInterval(window.setInterval(() => {
  ...
  async loadSettings() {
    this.storage = createSharedStorage(this.owner, this, this.deviceLocalExternalContexts);
  ...
  ensureWorkspaceServices(): Promise<PiWorkspaceServices> {
    ...
    const initialization = measureStartupPhase(
      'workspace',
      () => createPluginServiceGraph(this.owner, this),
    ).then(async (graph) => {
