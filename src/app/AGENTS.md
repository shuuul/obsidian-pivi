...
- **Keep chat performance tracing development-only and instance-owned.** `PiviPlugin.onload()` dynamically imports `chatPerformanceRecorder.ts` only in development, while production retains only the disabled controller contract. The plugin injects that recorder through `PiviViewHost` / `ImperativeChatAdapter` into each tab's `ChatState`; debug commands start, heap-sample, run the deterministic 64-chunk / 100KB Markdown stream through the real active-tab projection and Markdown adapter, run a fixed 10-tab / 20-switch in-memory workload, run isolated 5K indexed cold-open/older-page traces, and stop/export traces under `.pivi/perf-traces/`. Every workload suspends tab persistence and restores the original active tab. The switching workload never binds session files; the indexed workload copies the fixed fixture to a unique temporary session id and deletes its JSONL/index after the real upward-scroll paging path completes. An optional `.pivi/perf-scenario.txt` supplies the scenario label for CLI-driven runs. Unload disposes observers; tracing never enters settings or real user session JSONL.
