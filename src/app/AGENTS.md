...
- **Keep chat performance tracing development-only and instance-owned.** `PiviPlugin.onload()` dynamically imports `chatPerformanceRecorder.ts` only in development, while production retains only the disabled controller contract. The plugin injects that recorder through `PiviViewHost` / `ImperativeChatAdapter` into each tab's `ChatState`; debug commands start, heap-sample, run the deterministic 64-chunk / 100KB Markdown stream through the real active-tab projection and Markdown adapter, run a fixed 10-tab / 20-switch in-memory workload with tab persistence suspended, and stop/export traces under `.pivi/perf-traces/`. Synthetic turns and tabs are removed after each run, never bind session files, and never enter tab/session persistence. An optional `.pivi/perf-scenario.txt` supplies the scenario label for CLI-driven runs. Unload disposes observers; tracing never enters settings or session JSONL.
