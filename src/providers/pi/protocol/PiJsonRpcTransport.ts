...
export class PiJsonRpcTransport {
	...
	start(): void {
		...
		this.streamUnsubscribers.push(
			...
			subscribeStreamEvent(this.streams.output, "close", () => {
				if (!this.disposed) {
					this.dispose(
						new JsonRpcTransportClosedError("JSON-RPC output closed"),
					);
				}
			}),
	...
	dispose(
		error: Error = new JsonRpcTransportClosedError(
			"JSON-RPC transport disposed",
		),
	): void {
		if (this.disposed) {
			return;
