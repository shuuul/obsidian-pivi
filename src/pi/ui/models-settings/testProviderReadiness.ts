...
export async function testProviderReadiness(
  ...
  if (!auth) {
    return { ok: false, detail: `No credential resolved for ${providerId}.` };
  }
  ...
  if (!baseUrl) {
    return {
      ok: true,
      detail: `Credentials resolved from ${auth.source}; this provider has no endpoint URL to probe locally.`,
    };
  }
