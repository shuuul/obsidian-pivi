...
export class PiMcpBridge {
  ...
  private proxyTool: AgentTool | null = null;
  ...
  getProxyTool(): AgentTool {
    if (!this.proxyTool) {
      this.proxyTool = createPiMcpProxyTool(this);
    }
    return this.proxyTool;
  }

  getAgentTools(): AgentTool[] {
    return this.hasServers() ? [this.getProxyTool()] : [];
  }
  ...
  resolveActiveMentions(turn: PreparedChatTurn): Set<string> {
