...
Pi adaptor implementation: in-process `Agent`, streaming runtime, settings, and workspace services. Wired into `ProviderRegistry` / `ProviderWorkspaceRegistry` from `main.ts` at startup.

## Key Files

- `registration.ts` — `ProviderRegistration` for chat-facing services
...
- Bootstrap in `main.ts` calls `ProviderRegistry.install(piProviderRegistration)` and `ProviderWorkspaceRegistry.install(piWorkspaceRegistration)`
