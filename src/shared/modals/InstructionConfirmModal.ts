...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('obsius2-instruction-modal');
    this.setTitle('Add custom instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'obsius2-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'obsius2-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'obsius2-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'obsius2-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'obsius2-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'obsius2-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'obsius2-instruction-clarification-section' });
    this.clarificationEl.addClass('obsius2-hidden');
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'obsius2-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'obsius2-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'obsius2-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('obsius2-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'obsius2-instruction-confirmation-section' });
    this.confirmationEl.addClass('obsius2-hidden');

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'obsius2-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'obsius2-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'obsius2-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'obsius2-instruction-edit-container' });
    this.editContainerEl.addClass('obsius2-hidden');

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('obsius2-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'obsius2-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.obsius2-instruction-spinner');
  ...
  private showState(state: ModalState) {
    ...
    if (this.loadingEl) {
      this.loadingEl.toggleClass('obsius2-hidden', state !== 'loading');
    }
    if (this.clarificationEl) {
      this.clarificationEl.toggleClass('obsius2-hidden', state !== 'clarification');
    }
    if (this.confirmationEl) {
      this.confirmationEl.toggleClass('obsius2-hidden', state !== 'confirmation');
    }
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'obsius2-instruction-btn obsius2-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'obsius2-instruction-btn obsius2-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'obsius2-instruction-btn obsius2-instruction-edit-btn',
  ...
  private toggleEdit() {
    ...
    if (this.isEditing) {
      this.refinedDisplayEl?.addClass('obsius2-hidden');
      this.editContainerEl?.removeClass('obsius2-hidden');
      ...
      if (this.refinedDisplayEl) {
        ...
        this.refinedDisplayEl.removeClass('obsius2-hidden');
      }
      this.editContainerEl?.addClass('obsius2-hidden');
