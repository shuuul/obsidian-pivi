...
- `features/`: Obsidian UI for chat, settings, and inline edit. Must not import `src/pi/`; use `core/agent/PiAgentServices` and `AgentWorkspace`.
...
- `pi/bootstrap.ts` — installs Pi registrations into `PiAgentServices` and `AgentWorkspace`.
- `core/agent/PiAgentServices.ts` — chat-facing facade for runtimes, UI config, history/title/inline services.
